  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section className="relative bg-gradient-to-r from-primary via-primary/90 to-primary py-16">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="absolute top-4 left-4 z-10">
                {tyreData.ideal_for.map((vehicle) => (
                  <div
                    key={vehicle}
                    className="inline-flex items-center bg-card/90 px-2.5 py-1 rounded-md shadow-sm mr-2"
                  >
                    {/* Using enlarged version of car */}
                    {(vehicle === 'CAR') &&
                      <Car className="w-4 h-4 text-primary mr-1.5" />}
                    {vehicle === 'SUV' && <Car className="w-5 h-5 text-primary mr-1.5" />}
                    {vehicle === 'VAN' && <Car className="w-4 h-4 text-primary mr-1.5 scale-x-125" />}

                    {/* Labels */}
                    <span className="text-xs font-medium text-foreground">
                      {vehicle.charAt(0) + vehicle.slice(1).toUpperCase()}
                    </span>
                  </div>
                ))}
              </div>
              {/* Image Carousel */}
              <div className="relative bg-card/5 rounded-2xl p-8 backdrop-blur-sm">
                <div className="relative h-[300px] md:h-[400px] overflow-hidden rounded-xl">
                  ...
                  {tyreData.image_urls.length > 1 && (
                    <>
                      <button
                        onClick={prevImage}
                        className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-card/90 hover:bg-card shadow-lg transition-all duration-200 hover:scale-105 group"
                      >
                        <ChevronLeft className="w-6 h-6 text-primary group-hover:text-primary/80" />
                      </button>
                      <button
                        onClick={nextImage}
                        className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-card/90 hover:bg-card shadow-lg transition-all duration-200 hover:scale-105 group"
                      >
                        <ChevronRight className="w-6 h-6 text-primary group-hover:text-primary/80" />
                      </button>
                    </>
                  )}
                </div>
              ...
              <div className="text-primary-foreground space-y-6">
                ...
                <p className="text-primary-foreground/80 text-lg animate-fadeIn delay-200">
                  {tyreData.description}
                </p>
                <div className="flex flex-wrap gap-4 pt-4 animate-fadeIn delay-300">
                  <a href="tel:+917339099098" >
                    <button className="bg-card text-primary px-8 py-3 rounded-lg hover:bg-card/90 transition-colors">
                      Get Quote
                    </button>
                  </a>
                  {/* Brochure Button */}
                  {tyreData.brochure && (
                    <a
                      ...
                      className="border-2 border-card text-primary-foreground px-8 py-3 rounded-lg hover:bg-card/10 transition-colors"
                    >
                      View Brochure
                    </a>
                  )}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {tyreData.features.map((featureObj, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="flex items-start space-x-4 p-6 rounded-xl bg-secondary hover:bg-accent transition-colors duration-300"
                      style={{ animationDelay: `${index * 100}ms` }}
                    >
                      <Award className="w-6 h-6 text-primary flex-shrink-0 mt-1" />
                      <div>
                        <h4 className="font-semibold text-foreground">{featureTitle}</h4>
        ...
        <section className="py-12 bg-muted">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto space-y-8">
              {/* Size Selection */}
              <div className="bg-card rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-semibold text-foreground">Select Your Size</h2>
                  <button
                  ...
                  >
                    ...
                    <span className="absolute right-full mr-2 top-1/2 -translate-y-1/2 px-2 py-1 bg-foreground text-background text-sm rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                      Tyre Guide
                    </span>
                  </button>
                </div>
                    ...
                    <div className="bg-card rounded-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
                      <div className="p-6">
                        <div className="flex justify-between items-center mb-6">
                          <h3 className="text-xl font-semibold text-foreground">Tyre Guide</h3>
                          <button
                        ...
                        <div className="space-y-6">
                          {Object.entries(tyreGuide).map(([key, guide]) => {
                            ...
                            return (
                              <div key={key} className="p-4 bg-muted rounded-lg hover:bg-accent transition-colors duration-300">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {tyreData.specification_table.map((spec) => (
                    <button
                      ...
                      className={`p-4 text-sm border rounded-lg transition-all duration-300 hover:scale-105 ${selectedSize?.size === spec.size
              ...
              {selectedSize && (
                <div className="bg-card rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
                  <h2 className="text-2xl font-semibold mb-6 text-foreground">Specifications</h2>
                  <div className="grid md:grid-cols-3 gap-6">
                    {Object.entries(selectedSize).map(([key, value], index) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="relative p-4 bg-muted rounded-lg hover:bg-accent transition-colors duration-300 group"
                          style={{ animationDelay: `${index * 100}ms` }}
                        >
                          <div className="flex items-center justify-between">
                            <div className="text-muted-foreground text-sm mb-1 flex-1">
              ...
              {showSpecGuide && (
                <div className="fixed inset-0 bg-background/50 flex items-center justify-center z-50 p-4">
                  <div className="bg-card rounded-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
                    <div className="p-6">
                      <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-semibold text-foreground">Specification Guide</h3>
                        <button
                      ...
                      <div className="space-y-6">
                        {Object.entries(specificationGuide).map(([key, spec]) => (
                          <div
                            id={`spec-${key}`}
                            key={key}
                            className="p-4 bg-muted rounded-lg hover:bg-accent transition-colors duration-300"
                          >
        ...
        <section className="py-12 bg-card">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link
                href="/tyres"
                className="text-primary hover:text-primary/80 transition-all duration-300 inline-flex items-center gap-2 group"
              >
                View All
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {relatedProducts.map((product, index) => (
                <Link
                  href={product.url}
                  key={index}
                  className="group relative bg-card rounded-2xl p-6 transition-all duration-300 
                    hover:shadow-lg hover:-translate-y-1 border border-border hover:border-primary
                    focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                >
                  {/* Product Image */}
                  <div className="aspect-square rounded-xl bg-muted mb-4 p-6 flex items-center justify-center 
                    overflow-hidden group-hover:bg-accent transition-colors">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-foreground font-medium line-clamp-1 group-hover:text-primary 
                    transition-colors">
                      {product.name}
                    </h3>
                  ...
                  {product.specification_table && product.specification_table[0] && (
                    <div className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground bg-muted 
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <span className="text-sm font-medium text-primary group-hover:translate-x-1 
                  ...
                  {/* Hover Indicator */}
                  <div className="absolute inset-x-0 bottom-0 h-1 bg-primary transform scale-x-0 
