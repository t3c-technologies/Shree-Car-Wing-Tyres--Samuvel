...
const TwoWheelerPage = () => {
  ...
  const filterCategories = {
    ...
    usage: {
      ...
      options: [
        ...
        { id: "Street Sport", label: "Street Sport" },
  ...
  useEffect(() => {
    const fetchTyres = async () => {
      try {
        ...
        const twoWheelerTyres = data.filter((tyre) => tyre.group === "2W");
    ...
    const applyFilters = () => {
      const filtered = tyres.filter((tyre) => {
        ...
        const matchesBikeType =
          selectedFilters.bikeType === "all" ||
          tyre.ideal_for.includes(selectedFilters.bikeType);

        const matchesUsage =
          selectedFilters.usage.length === 0 ||
          selectedFilters.usage.some((usage) =>
            Array.isArray(tyre.usage)
              ? tyre.usage.some((tyreUsage) => usage.includes(tyreUsage))
              : usage.includes(tyre.usage)
          );


        const matchesPosition =
          selectedFilters.position === "all" ||
          tyre.tyre_position === selectedFilters.position;

        const matchesConstruction =
          selectedFilters.construction.length === 0 ||
          selectedFilters.construction.some((type) =>
            Array.isArray(tyre.tyre_construction)
              ? tyre.tyre_construction.some((construction) => type.includes(construction))
              : type.includes(tyre.tyre_construction)
          );
  ...
  const handleFilterChange = useCallback((category, value, remove = false) => {
    setSelectedFilters((prev) => {
      if (filterCategories[category].type === "radio") {
        return { ...prev, [category]: remove ? "all" : value }; // Reset to "all" if removing
      }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 bg-gradient-to-r from-blue-900 to-blue-800 rounded-xl p-8 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl">
            <h1 className="text-3xl md:text-4xl font-bold mb-4">Two Wheeler Tyres</h1>
