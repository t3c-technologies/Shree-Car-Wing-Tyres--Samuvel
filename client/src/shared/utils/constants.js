...
export const getFilterDisplayLabel = (filterKey, filterValue) => {
  switch (filterKey) {
    case 'category':
      const categoryMap = {
      ...
      return categoryMap[filterValue] || filterValue;

    case 'brand':
      const brandInfo = BRAND_INFO.find(brand =>
        brand.name.toLowerCase() === filterValue.toLowerCase()
      );
      return brandInfo ? brandInfo.name : filterValue;
    ...
    case 'rating':
      return `${filterValue}+ Star${filterValue > 1 ? 's' : ''}`;

    case 'size':
      return filterValue;

    case 'loadIndex':
      return `Load Index: ${filterValue}`;

    case 'speedRating':
      return `Speed Rating: ${filterValue}`;
    ...
    case 'availability':
      ...
      return availabilityMap[filterValue] || filterValue;
...
export const getFilterCategoryLabel = (filterKey) => {
  ...
  return categoryLabels[filterKey] || filterKey;
};